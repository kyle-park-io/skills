...
유저 스코프는 **스킬 30개를 넘기지 않는다.** 플러그인 개수가 아니라 스킬 개수로 센다. MCP 전용 플러그인(스킬 0개)은 이 예산에 거의 안 들어간다. 툴 스키마가 컨텍스트에 상주하지 않고 쓸 때 불려오기 때문이다. 반대로 `~/.claude/skills/`에 걸어 둔 스킬은 플러그인이 아니어도 매 세션 상주하므로 같이 센다. 자세한 배경과 도메인별 배치안: [`docs/SETUP-GUIDE.md`](docs/SETUP-GUIDE.md)
