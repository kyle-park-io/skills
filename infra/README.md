...
아래는 역할 카탈로그이며 `infra@kyle-skills`에 묶여 오는 의존성이 아니다.
2026-09-10 현재 Codex 공식 카탈로그에서는 `sentry`, `github`, `cloudflare`,
`vercel`, `datadog`를 확인했다. 나머지는 하네스별 설치 경로를 따로 검증한다.
...
이 도메인의 `skills/`는 아직 비어 있다. 현재 플러그인을 설치해도 인프라
