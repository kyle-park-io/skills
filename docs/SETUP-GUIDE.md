...
병합은 프리셋에 있는 키만 덮어쓰므로, 지운 항목은 이미 켜진 머신에서 그대로 살아 있다. 지우면 "관심 없음", `false`면 "꺼라"다. **그 `false`는 설치된 플러그인에만 적는다 (2026-09-10 추가).** 설치되지 않은 플러그인에 적으면 "프로젝트 설정이 켰는데 설치가 안 됐다"는 로드 에러가 세션마다 난다. 부트스트랩이 설치 목록을 넘겨 주므로 병합이 걸러 낸다. 프로젝트 스코프가 유저 스코프를 덮으므로 해당 레포에서는 정상적으로 켜진다. **유저 캐시에 설치돼 있을 때만이다.**
