...
1. **내장·기존 스킬과 겹치는가.** 하네스 내장 16개와 superpowers 14개가 이미 상당 부분을 덮는다. 겹치면 만들지 않는다. 목록은 [`docs/SETUP-GUIDE.md`](docs/SETUP-GUIDE.md) §4.
2. **어느 도메인인가.** 도메인 선택이 곧 스코프 결정이다. 유저 스코프 도메인에 넣으면 그 스킬은 모든 세션에 상주한다.
3. **`description` 이 기존 스킬과 구별되는가.** 트리거 상황이 겹치면 둘 다 안 뽑힌다.

세 번째가 실패의 주된 원인이다. 감으로 고치지 말고 `skill-creator` 의 eval 과 변동성 분석으로 측정한다.
