...
**규칙은 "ASCII 만"이 아니다.** jarvis 히스토리는 영어 제목 안에서 `—` (U+2014) 와 `→` (U+2192) 를 쓴다 (`feat: Phase 2 execution — jarvis do → draft PR`). ASCII 로 잡으면 그 레포 자기 커밋 20개 중 3개가 떨어진다. herald 가 실제로 당한 것은 한글이었으므로 규칙도 한글/CJK 여야 한다 — 검사 대상이 "비-ASCII"가 아니라 "영어가 아닌 것"이라는 게 요점이다.
